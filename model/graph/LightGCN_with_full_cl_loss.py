...
class LightGCN(nn.Module):
    ...
    def forward(self, batch=None):
        ...
        cl_loss = self.cal_cl_loss(rneg_gcn_embs,neg_gcn_embs,item_gcn_emb[pos_item])*self.cl_weight
    ...
    def cal_cl_loss(self, random_item_view_1,hard_item_view_2,pos_item_view_3):
        # torch.Size([2048, 1, 3, 64])
        neg_e_random = self.pooling(random_item_view_1.view(-1, random_item_view_1.shape[2], random_item_view_1.shape[3]))
        neg_e_hard = self.pooling(hard_item_view_2.view(-1, hard_item_view_2.shape[2], hard_item_view_2.shape[3]))
        neg_e_pos = self.pooling(pos_item_view_3)
        ...
        item_cl_loss = self.InfoNCE(neg_e_random,neg_e_hard ,neg_e_pos, self.temperature)
        return  item_cl_loss

    def InfoNCE(self,view1, view2,view3, temperature):
